<?php
	...
	function myalbum_admin_form_export() {
		$irs = $GLOBALS['xoopsDB']->query( "SELECT c.imgcat_id,c.imgcat_name,c.imgcat_storetype,COUNT(i.image_id) AS imgcat_sum FROM ".$GLOBALS['xoopsDB']->prefix("imagecategory")." c NATURAL LEFT JOIN ".$GLOBALS['xoopsDB']->prefix("image")." i GROUP BY c.imgcat_id ORDER BY c.imgcat_weight" ) ;
		$imgcat_options = '' ;
...
<input type='submit' name='imagemanager_export' value='"._GO."' onclick='return confirm(\""._AM_MB_EXPORTCONFIRM."\");' />
	...
	function myalbum_admin_form_import_myalbum(){
		...
		while( list( $src_dirname ) = $GLOBALS['xoopsDB']->fetchRow( $mrs ) ) {
			...
			if( ! $GLOBALS['xoopsUser']->isAdmin( $module->getVar('mid') ) ) continue ;
